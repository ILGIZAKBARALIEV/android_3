...
class AnimalAdapter:Adapter<AnimalAdapter.ViewHolder>() {
    val animalList = arrayListOf<AnimalModel>(
        AnimalModel(title= "Ilgiz",desc= "0990442484", img = "https://static.vecteezy.com/system/resources/previews/057/068/323/non_2x/single-fresh-red-strawberry-on-table-green-background-food-fruit-sweet-macro-juicy-plant-image-photo.jpg"),
    ...
    class ViewHolder(private val binding: ItemAnimalBinding): RecyclerView.ViewHolder(binding.root){
        fun onBind(animalModel:AnimalModel){
            binding.tvTitile.text = animalModel.title
            binding.descTitile.text = animalModel.desc
            Glide.with(binding.tvImage).load(animalModel.img).into(binding.tvImage);
